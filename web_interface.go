...
// Global CUDA status for WebSocket broadcasting
var globalCudaEnabled *bool

// Status WebSocket broadcaster
type StatusBroadcaster struct {
	clients    map[*websocket.Conn]bool
	register   chan *websocket.Conn
	unregister chan *websocket.Conn
	broadcast  chan StatusData
	mutex      sync.RWMutex
}

type StatusData struct {
	CudaEnabled    bool           `json:"cudaEnabled"`
	Params         TorusParams    `json:"params"`
	AsteroidParams AsteroidParams `json:"asteroidParams"`
...
func newStatusBroadcaster() *StatusBroadcaster {
	return &StatusBroadcaster{
		...
		broadcast:  make(chan StatusData),
	}
}
...
func (sb *StatusBroadcaster) run() {
	...
	for {
		...
		case client := <-sb.register:
			...
			sb.mutex.Unlock()
			LogInfo("StatusWS", "New client connected to status stream")

			// Send initial status to new client
			sb.sendStatusToClient(client)
		...
		case client := <-sb.unregister:
			...
			sb.mutex.Unlock()
			LogInfo("StatusWS", "Client disconnected from status stream")
...
func (sb *StatusBroadcaster) sendStatusToClient(client *websocket.Conn) {
	status := StatusData{
		CudaEnabled:    globalCudaEnabled != nil && *globalCudaEnabled,
		Params:         currentParams,
		AsteroidParams: currentAsteroidParams,
		CurrentScene:   currentScene,
		FPS:            GetCurrentFPS(),
		WindowWidth:    func() int { w, _ := GetWindowSize(); return w }(),
		WindowHeight:   func() int { _, h := GetWindowSize(); return h }(),
	}

	if err := client.WriteJSON(status); err != nil {
		sb.unregister <- client
	}
}
...
<body>
    <div class="main-panel">
        ...
        <div class="scene-panel" id="asteroidsPanel">
            ...
            <br>
            <strong>Performance:</strong> <span id="fpsDisplay">-- FPS</span>
            <br>
            <strong>Viewport:</strong> <span id="windowSize">800 x 600</span> (resizable)
        </div>
    </div>
    ...
    <script>
        ...
        connectWebSocket();  // For logs
        connectStatusWebSocket();  // For status updates
        ...
        function connectStatusWebSocket() {
            const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
            const statusWsUrl = protocol + '//' + window.location.host + '/ws-status';
            
            statusWs = new WebSocket(statusWsUrl);
            
            statusWs.onopen = function() {
                statusConnected = true;
                addLogEntry('StatusWS', 'info', 'Connected to status stream');
            };
            ...
            statusWs.onmessage = function(event) {
                try {
                    ...
                    document.getElementById('renderMode').textContent = 
                        statusData.cudaEnabled ? 'CUDA Accelerated' : 'CPU Fallback';
                    
                    // Update FPS display
                    const fps = statusData.fps ? statusData.fps.toFixed(1) : '--';
                    document.getElementById('fpsDisplay').textContent = fps + ' FPS';
                    
                    // Update window size display
                    if (statusData.windowWidth && statusData.windowHeight) {
                        document.getElementById('windowSize').textContent = 
                            statusData.windowWidth + ' x ' + statusData.windowHeight;
                    }
                } catch (e) {
                    addLogEntry('StatusWS', 'error', 'Failed to parse status message: ' + event.data);
                }
            };
            
            statusWs.onclose = function() {
                statusConnected = false;
                addLogEntry('StatusWS', 'warning', 'Disconnected from status stream, attempting to reconnect...');
                setTimeout(connectStatusWebSocket, 3000);
            };
            
            statusWs.onerror = function(error) {
                addLogEntry('StatusWS', 'error', 'Status connection error: ' + error);
            };
        }
    </script>
</body>
...
func StartWebServer(params *TorusParams, cudaEnabled *bool) {
	// Set global CUDA status for WebSocket broadcasting
	globalCudaEnabled = cudaEnabled

	// Initialize and start status broadcaster
	...
	http.HandleFunc("/ws", wsLogHandler)
	...
	http.HandleFunc("/ws-status", func(w http.ResponseWriter, r *http.Request) {
		...
		statusBroadcaster.register <- conn

		// Keep connection alive and handle disconnection
		defer func() {
			statusBroadcaster.unregister <- conn
			conn.Close()
		}()
