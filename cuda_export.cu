...
__declspec(dllexport) void updateTorusVerticesCUDA(float* vertices, int numVertices, float time, float majorRadius, float minorRadius) {
    if (!cudaInitialized || !ensureDeviceMemory(numVertices)) {
        printf("CUDA not initialized or memory allocation failed\n");
        return;
    }
    
    size_t vertexSize = numVertices * 6 * sizeof(float);
    if (cudaMemcpy(d_vertices, vertices, vertexSize, cudaMemcpyHostToDevice) != cudaSuccess) {
        printf("Failed to copy vertices to device\n");
        return;
    }
    ...
    if (cudaMemcpy(vertices, d_vertices, vertexSize, cudaMemcpyDeviceToHost) != cudaSuccess) {
        printf("Failed to copy vertices from device\n");
        return;    }
    
    // printf("CUDA vertex update completed for %d vertices\n", numVertices);
}

__declspec(dllexport) void updateTorusColorsCUDA(float* colors, int numVertices, float time) {
    if (!cudaInitialized || !ensureDeviceMemory(numVertices)) {
        printf("CUDA not initialized or memory allocation failed\n");
        return;
    }
    ...
    size_t colorSize = numVertices * 3 * sizeof(float);
    if (cudaMemcpy(d_colors, colors, colorSize, cudaMemcpyHostToDevice) != cudaSuccess) {
        printf("Failed to copy colors to device\n");
        return;
    }
    ...
    if (cudaDeviceSynchronize() != cudaSuccess) {
        printf("CUDA color kernel execution failed\n");
        return;
    }
    
    if (cudaMemcpy(colors, d_colors, colorSize, cudaMemcpyDeviceToHost) != cudaSuccess) {
        printf("Failed to copy colors from device\n");
        return;    }
    
    // printf("CUDA color update completed for %d vertices\n", numVertices);
}
